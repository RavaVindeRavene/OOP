...
class OverflowException { //  visu zemam ieks sim iekavam var izdzest };
public:
    OverflowException() {
...
short ComputerNetwork::GetMaxRam() {
    short maxRam = Nodes[0]->GetApjoms();
    for (int i = 1; i < Length; i++) {
        if (maxRam < Nodes[i]->GetApjoms()) maxRam = Nodes[i]->GetApjoms();
    };
    return maxRam;
}
...
int main() {
    ...
    cout << "\n\nDefault maximal length (from CLASS): " << ComputerNetwork::GetDefaultMaxLength() << "." << endl;
    cout << "Default maximal length (from OBJECT): " <<Network->GetDefaultMaxLength() << "." << endl;
    cout << "Maximal length: " << Network->GetMaxLength() << "." << endl;
    cout << "Current length: " << Network->GetLength() << "." << endl;

    cout << "Maksimalais RAM " << endl;
    cout << Network->GetMaxRam() << endl;
    delete NPC1;
    Network->Print();
    delete Network;
